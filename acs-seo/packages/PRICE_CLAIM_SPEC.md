...
**Claim row:** CR-03 (OWNER_REQUIRED)
...
**State:** both `REVISE`. Neither can reach PASS here: the blocker is SRC-06, the ACS
...
```sh
node acs-seo/tools/claim-scan.mjs <draft-dir>
```

Six rules report CR-03. They cover more than a currency figure, because a price claim
...
| Form | When | Shape |
|---|---|---|
| **A — No price** | Default. No SRC-06 entry for this item | Describe what drives cost, never a figure: "ต้นทุนรวมขึ้นกับจำนวนจุดสแกน อุปกรณ์ที่เลือก และงานติดตั้ง — ขอใบเสนอราคาตามขอบเขตงานจริงได้" |
| **B — Priced, dated, scoped** | SRC-06 entry exists | "ราคา 18,500 บาทต่อเครื่อง ยังไม่รวม VAT (ราคา ณ วันที่ 2026-09-01)" |
...
1. Copy `templates/price_evidence.json` to `packages/queue-16/<item>-price-evidence.json` (or `queue-18/`), one per published figure.
...
- [ ] `claim-scan.mjs <draft>` reports zero CR-03 findings that are not backed by a record
...
**SRC-06, the ACS price list with effective dates.** Without it every flagged sentence
