...
**Claim rows:** CR-01 (EVIDENCE_REQUIRED), CR-07 (vendor specification)
**State:** `REVISE`, and it **cannot reach PASS in this session.** Unlike #17 and #21, this
package's blocker is a source that does not exist yet: SRC-03, the vendor datasheets.
...
```sh
node acs-seo/tools/claim-scan.mjs <draft-dir> --strict
```
...
`templates/datasheet_evidence.json` therefore makes `measured_conditions` mandatory, and
...
1. Copy `templates/datasheet_evidence.json` to `packages/queue-4/<claim>-evidence.json`, one per flagged sentence.
2. Fill `product` from the **ACS product list (SRC-02)**. A datasheet for a product ACS does not sell evidences nothing.
...
- [ ] `claim-scan.mjs <draft> --strict` exits 0
...
- [ ] Tier 0 mechanical check passes; Tier 2B CLAIM QA run on every CR-01 and CR-07 sentence
- [ ] `package_status.json` still `DRAFT_PENDING_REVIEW`; rendering gate still uncleared

## 7. Blocked on — the one remaining input

**SRC-03, vendor datasheets** (Brady / Honeywell / TSC), paired with **SRC-02, the ACS
