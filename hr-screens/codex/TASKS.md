...
- **Dependencies:** AUTH-002, AMI-001 · ยึด `D-AMI-01` และ `D-AMI-02`
...
## คำตัดสินที่ปลดล็อกงานชุดนี้

เจ้าของระบบสั่งเมื่อ 2026-09-24 ว่า **ไม่ต้องรอคำตัดสินรายข้อ ให้เดินงานต่อ**
โดยใช้ข้อเสนอของฝั่งรีวิวสถาปัตยกรรม บันทึกเต็มอยู่ใน `contract/decisions.json`
พร้อมที่มา ความเสี่ยงที่ยังค้าง และว่าข้อไหนกลับทางได้

| | คำตัดสิน | บังคับด้วย | กลับทางได้ |
|---|---|---|---|
| **D-AMI-01** | AMI ไม่ผูกกับวิธี auth ใดเลย อ่าน session ที่แพลตฟอร์มให้มา และคาดหวังแค่ identity ที่ยืนยันแล้วกับ `role_id` | `server-contract.json` SR-06 | ได้ |
| **D-AMI-02** | Keeper เป็นชั้นฝั่ง server ที่ประเมิน**หลัง** `authorize()` ผ่าน อยู่ใน `packages/core` มี RLS เป็น defence in depth | SR-03 · `enums.json#access_verdict` | **ไม่ได้** |
| **D-AMI-03** | `/ami/<slug>` · noindex,nofollow · ไม่อยู่ใน sitemap · **ไม่วางใต้ `/admin/`** | `screens.json#route_base` | ได้ |
| **D-AMI-04** | ฉบับ en ผ่านการรีวิวความหมายแล้ว ใช้ได้ | `ui-strings.json#en_status` | ได้ |

### ทำไม D-AMI-01 ถึงปลดล็อกได้โดยไม่ต้องตัดสินวิธีเข้าสู่ระบบ

บรีฟระบุเองว่า *"งานของคุณคือหน้าจอส่วนบุคคลหลังเข้าสู่ระบบ 6 แบบ ไม่ต้องสร้างระบบบัญชีใหม่"*
AMI อยู่**หลัง**การเข้าสู่ระบบ จึงไม่ได้เลือกวิธี auth และไม่ควรมีโค้ด auth ของตัวเอง

ความขัดแย้งระหว่างบรีฟ (อีเมล + รหัสเริ่มต้น) กับ `docs/03-security-events-admin.md`
(*No passwords in V1* ใช้ OTP) ยังเป็นเรื่องจริงและต้องตัดสินสักวัน **แต่อยู่ชั้นเหนือ AMI**
การเลือก session provider จริงเป็นงาน integration หนึ่งบรรทัด ไม่ใช่การออกแบบ AMI ใหม่

### ทำไม D-AMI-02 ถึงกลับทางไม่ได้

ถ้า Keeper อยู่ฝั่ง UI มันคือ**การซ่อน ไม่ใช่การกั้น** — เปิด DevTools หรือยิง API ตรงก็ข้ามได้
และหลักข้อสี่ *The Keeper controls disclosure* จะเหลือแค่คำโฆษณา

### ความเสี่ยงที่ยังค้าง ไม่ได้หายไป

`decisions.json#open_risks` บันทึกไว้สองข้อ ข้อที่สำคัญคือ **ข้อความ UI เอ่ยชื่อระบบภายในตรง ๆ**
(THE KEEPER, MATCHMAKER-1, AMI Core, xAI, GROK-xx) ถ้าหน้าจอถูก screenshot ออกนอกองค์กร
โครงสร้าง AI ภายในจะติดไปด้วย — ไม่บล็อกเพราะบรีฟกำหนดเองว่าทุกจอต้องแสดง call sign และ Grok
และแก้ได้ทันทีที่ `ui-strings.json` เพราะเป็นข้อความอย่างเดียว
