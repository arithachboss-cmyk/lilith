...
The API connection reports read permission, while SSH dry-run allows pushing the task branch. Actual PR/CI permissions remain to be verified.
...
STATUS: local acceptance passed; task not Done until CI passes.
...
BLOCKERS: none for local ARCH-001 implementation; PR/CI not verified yet.
NEXT: publish PR, observe CI and submit for review.
OWNER APPROVAL: review the concrete ARCH-001 PR before dependency merge; no fee decision needed in this task.

READY_FOR_REVIEW: true (local implementation; hosted CI still pending).
